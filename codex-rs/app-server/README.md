...
- `mcpToolCall` — `{id, server, tool, status, arguments, connectorId, linkId, mcpAppResourceUri?, pluginId, result?, error?}` describing MCP calls; `connectorId` and `linkId` identify the trusted MCP app connector link, while `tool` identifies the invoked action and `mcpAppResourceUri` points to the widget template. `status` is `inProgress`, `completed`, or `failed`.
