...
- `mcpToolCall` — `{id, server, tool, status, arguments, connectorId, mcpAppResourceUri?, pluginId, result?, error?}` describing MCP calls; `connectorId` identifies the trusted MCP app connector. `status` is `inProgress`, `completed`, or `failed`.
