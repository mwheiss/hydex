...
async fn local_offload_responses_request_omits_codex_control_plane_metadata() {
    ...
    let tools = serde_json::to_value(tools).expect("local tools serialize");
    let tools = tools.as_array().expect("local tools are an array");
...
#[test]
fn compact_subagent_turn_source_routes_local_with_offload_configured() {
    let client =
        test_model_client_with_local_offload(SessionSource::SubAgent(SubAgentSource::Compact));
    let responses_metadata = test_responses_metadata_for_client(
        &client,
        Some("turn-1"),
        format!("{}:0", client.state.thread_id),
...
#[test]
fn ordinary_subagent_sources_route_local_with_offload_configured() {
    for session_source in [
        SessionSource::SubAgent(SubAgentSource::Review),
        SessionSource::SubAgent(SubAgentSource::Other("delegate".to_string())),
        SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            parent_thread_id: ThreadId::new(),
            depth: 1,
            agent_path: None,
        ...
        assert!(
            client
                .route_for_responses_request(&responses_metadata)
                .is_local_offload(),
            "expected {session_source:?} to route local",
        );
        assert!(
            client
                .new_session()
...
#[test]
fn guardian_auto_review_routes_local_with_offload_configured() {
    let client = test_model_client_with_local_offload(SessionSource::SubAgent(
...
#[tokio::test]
async fn guardian_auto_review_local_route_omits_codex_control_plane_metadata() {
    let client = test_model_client_with_local_offload(SessionSource::SubAgent(
        SubAgentSource::Other(crate::guardian::GUARDIAN_REVIEWER_NAME.to_string()),
    ));
    let client_session = client.new_session();
...
fn memory_mode_local_routes_memory_requests_and_consolidation_local() {
    ...
    let subagent_consolidation_client = test_model_client_with_local_offload_config_and_memory_mode(
        SessionSource::SubAgent(SubAgentSource::MemoryConsolidation),
        ModelOffloadCompactionPolicy::Local,
        ModelOffloadMemoryMode::Local,
    );
    let turn_metadata = test_responses_metadata_for_client(
        &subagent_consolidation_client,
        Some("turn-1"),
        format!("{}:0", subagent_consolidation_client.state.thread_id),
        None,
        TestCodexResponsesRequestKind::Turn,
    );
    assert!(
...
#[tokio::test]
async fn compact_subagent_compaction_follows_root_compaction_policy() {
    let local_client = test_model_client_with_local_offload_config(
        SessionSource::SubAgent(SubAgentSource::Compact),
        ModelOffloadCompactionPolicy::Local,
    );
    local_client.seed_offload_ever_used(true);
    let local_metadata = compaction_responses_metadata_for_client(
        &local_client,
    ...
    assert!(
        local_client
            .route_for_responses_request(&local_metadata)
            .is_local_offload()
    );
    assert_eq!(
        request_model_for_metadata(&local_client, &local_metadata).await,
        "local-responses-model"
    );

    let primary_client = test_model_client_with_local_offload_and_api_primary(
        SessionSource::SubAgent(SubAgentSource::Compact),
        ModelOffloadCompactionPolicy::Primary,
    );
    primary_client.seed_offload_ever_used(true);
    let primary_metadata = compaction_responses_metadata_for_client(
        &primary_client,
        CompactionImplementation::Responses,
    );
    ...
    assert!(
        !primary_client
            .route_for_responses_request(&primary_metadata)
            .is_local_offload()
    );
    assert_eq!(
        request_model_for_metadata(&primary_client, &primary_metadata).await,
        "gpt-test"
    );
}
