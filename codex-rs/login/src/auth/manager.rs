...
impl CodexAuth {
    async fn from_auth_dot_json(
        ...
        if auth_mode == ApiAuthMode::AgentIdentity {
            ...
            let base_url = chatgpt_base_url
                .unwrap_or(ChatGptEnvironment::Production.chatgpt_base_url())
                .trim_end_matches('/')
                .to_string();
            ...
            match agent_identity {
                ...
                AgentIdentityStorage::Record(record) => {
                    ...
                    .await?;
                    let storage = AgentIdentityStorage::Record(auth.record().clone());
    ...
    pub async fn from_agent_identity_jwt(
        ...
        Ok(Self::AgentIdentity(
            AgentIdentityAuth::from_record_with_registered_task(record, &resolved_authapi_base_url)
                .await?,
        ))
    }
    ...
    async fn ensure_chatgpt_agent_identity(
            ...
            && public_key_ssh_from_private_key_pkcs8_base64(&record.agent_private_key).is_ok()
        {
            let auth = map_retry_exhaustion_to_bootstrap_unavailable(
                ...
                AgentIdentityAuth::from_record_with_registered_task(
                    record,
                    &agent_identity_authapi_base_url,
                )
                .await,
