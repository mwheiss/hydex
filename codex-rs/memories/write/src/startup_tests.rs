...
async fn memories_startup_memory_mode_local_uses_offload_route() -> anyhow::Result<()> {
    ...
    let request = run_memory_phase_one_model_request_test_with_config(
        ...
        move |config| {
            ...
            config.model_offload.context.context_window = Some(64_000);
        },
    )
    .await?;
...
#[tokio::test]
async fn memories_startup_local_memory_requires_completed_response_before_commit()
-> anyhow::Result<()> {
    let server = start_mock_server().await;
    let home = Arc::new(TempDir::new()?);
    let local_provider =
    ...
    let response = mount_sse_once(
        ...
        sse(vec![
            ...
            ev_assistant_message(
                "msg-phase1",
                r#"{"raw_memory":"must not persist","rollout_summary":"rollout summary","rollout_slug":"startup-incomplete"}"#,
            ),
        ]),
    )
    .await;
