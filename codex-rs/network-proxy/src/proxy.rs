...
mod tests {
    ...
    #[tokio::test]
    async fn child_env_snapshot_virtualizes_credentials() {
        let settings = NetworkProxySettings {
        ...
        let proxy = NetworkProxy::builder()
            ...
            .await
            .unwrap();
        let snapshot = proxy.child_env_snapshot();
        let mut env = HashMap::from([("OPENAI_API_KEY".to_string(), "sk-real".to_string())]);

        assert!(
            snapshot
                .prepare_child_env(&mut env, Path::new("/"), |_| true)
                .is_empty()
        );
        assert_eq!(
            env.get("OPENAI_API_KEY"),
            Some(&"sk-codex-dummy-0000".to_string())
        );
    }
