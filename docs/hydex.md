...
source is eligible. CLI, VS Code, exec, MCP, custom, unknown, and ordinary
subagent session sources are eligible. Ordinary subagents include review
subagents, spawned/thread subagents, Guardian auto-review, and other
agent-labeled workers. Compact subagent sources are source-eligible too; when
the request is a compaction request, the same root compaction policy and
injection behavior decide whether it runs local or primary. Memory-consolidation
subagents follow `memory_mode`, like internal memory consolidation. Other
internal and control-plane workflows stay primary by default.
...
behavior. This routing applies to detached memory generation and memory
consolidation session sources.
