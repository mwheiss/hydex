...
## Upstream parity hardening after rebase

The July 2026 rebase audit tightened the patch boundary so primary requests keep
the upstream behavior even when Hydex offload is configured:

- Tool planning now takes an explicit wire target. Primary requests retain the
  upstream hosted-tool surface; only a request already classified as local uses
  the local `web.run` namespace representation and local wire transforms.
- Local assistant-state compaction prompts, projection, and validation are
  selected only when the compaction request actually routes local. Primary and
  custom-primary baseline compaction retain upstream user-summary behavior.
- Remote-compaction model provenance is persisted only after offload has
  participated in the branch, and Hydex route-resolution logging starts only
  after an actual local route. Configuring offload without using it therefore
  does not alter primary history or primary-route logs.
- The plugin refresh is maintained as a semantic patch: it derives and
  namespaces VS Code-local registration IDs from each upstream package while
  preserving OpenAI/Statsig/server-facing Codex identity and feature gates.
...
5. Route classification is intentionally smaller than the conceptual route enum.
   ...
   - Ordinary subagent turn sources are eligible for local offload when offload is
     enabled. This includes review subagents, spawned/thread subagents, Guardian
     auto-review, and other agent-labeled workers.
   - Compact subagent sources are source-eligible too. If the request is a
     compaction request, it follows the same root compaction policy and injection
     behavior. Memory-consolidation subagents follow the memory routing mode.
...
7. Memory routing is configurable.
   - `[model_offload] memory_mode = "off" | "primary" | "local"` controls memory
     generation routing without changing memory reads or deleting existing
     memories.
   - `primary` passes `ModelOffloadConfig::default()` to memory generation and
     preserves primary-provider behavior.
   - `local` passes the active Hydex offload config into memory generation and
     routes memory generation and memory-consolidation session sources through
     the local provider.
...
17. Local assistant-state compaction is the Hydex default.
   - `[model_offload.compaction] local_handoff_role = "assistant_state"` uses the
     bundled assistant-continuation prompt and stores the raw compacted payload as
     structured assistant history in replacement history.
   - `local_handoff_role = "user_summary"` is the explicit legacy compatibility
     mode and preserves the older user-summary handoff behavior.
